...
import {StyleSheet, Text, TouchableOpacity, Alert, ImageBackground, Image,
  SafeAreaView, View, Dimensions, ScrollView, Platform,
} from 'react-native';
...
export default function WDisplayImage({ route, navigation }){
            ...
            headerRight: () => fourImages.length === 4 ? (
                <TouchableOpacity onPress={() => Alert.alert('Proceed to next step?', 
                    ...
                    [{ text: 'Cancel'}, { text: 'OK', onPress: () => navigation.replace(
                        'WChooseFrame', {fourImages: fourImages})},])}>
    ...
    /**
     * Alert handling for both mobile and web platforms.
     */
    const handleAlert = () => {
        if (Platform.OS === 'web') {
            window.confirm('Proceed to next step? Continue if you\'re satisfied with your selection.') && navigation.replace('ChooseFrame', {fourImages: fourImages});
        } else {
            Alert.alert('Proceed to next step?', 
                "Continue if you're satisfied with your selection.",
                [{ text: 'Cancel'}, { text: 'OK', onPress: () => navigation.replace('ChooseFrame', {fourImages: fourImages})}],
            );
        }
    };
    ...
    return (
            ...
            <SafeAreaView  style={styles.homeScreen}>
                ...
                <View style={{marginTop: 10, alignItems: 'center', position: 'relative'}}>
                    <View style={{height: 18, borderWidth: 1, borderColor: 'red'}}></View>
                    
    
                    {fourImages.map((uri, index) => {
                        // calc position based on index
                        const posXLeft = (index % 2) * (390/2) + 14; // - moves left, + moves right
                        const posXRight = (index % 2)  * (390/2) - 11; //+ moves left, - moves right
                        const posYBottom = (index < 2) * (390 / 2) + 275; // + moves down, - moves up
                        const posYTop = (index < 2) * (390 / 2) - 194; //- moves down, + moves up
                    ...
                    <Image source={require('../assets/images/WFrames/Wframe_01.png') }
                        style={{ 
                            width: 192, 
                            height: 576,
                            position: 'absolute',
                ...
                <View style={{ width: '700px', position: 'absolute', marginTop: 640, }}>
